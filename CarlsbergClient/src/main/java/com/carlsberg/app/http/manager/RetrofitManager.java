package com.carlsberg.app.http.manager;
...
import com.carlsberg.app.http.service.VisitService;
    ...
    private Interceptor mLoggingInterceptor = new Interceptor() {
        ...
        public Response intercept(Chain chain) throws IOException {
            ...
            if (contentLength != 0) {
                ...
                if((HttpConstants.LOGIN_STATUS_DISABLED+"").equals(code)){ //登录失效
                    CarlsbergAppcation.getInstance().getHandler().post(new Runnable() {
                        @Override
                        public void run() {
                            CarlsbergAppcation.getInstance().setUser(null);
                            ShiHuiActivityManager.getInstance().cleanActivity();
//                            Intent intent = new Intent(CarlsbergAppcation.getInstance(), LoginActivity.class);
//                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                            CarlsbergAppcation.getInstance().startActivity(intent);
                            ToastUtil.showLongToast(CarlsbergAppcation.getInstance(), "由于您长时间未使用"+ CarlsbergAppcation.getInstance().getResources().getString(R.string.app_name) +", 登录状态失效, 请重新登录!");
                        }
                    });
                    KLog.e("####################登录状态失效#######################");
                }else if((Constant.ACCOUNT_ERROR+"").equals(code)){
                    KLog.e("####################登录状态改变, 删除或冻结#######################");
                    CarlsbergAppcation.getInstance().getHandler().post(new Runnable() {
                        @Override
                        public void run() {
...
//                            intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
//                            CarlsbergAppcation.getInstance().startActivity(intent);
                            ToastUtil.showLongToast(CarlsbergAppcation.getInstance(), "登录状态改变, 请重新登录!");
                        }
                    });
                }
                KLog.e(jo.toString());
        ...
        if (sOkHttpClient == null) {
            synchronized (RetrofitManager.class) {
                if (sOkHttpClient == null) {
                    ...
                    // 指定缓存路径,缓存大小100Mb
                    Cache cache = new Cache(new File(CarlsbergAppcation.getInstance().getCacheDir(), "HttpCache"),
                            1024 * 1024 * 100);
...
*/
    ...
    public VisitService getVisitService() {
        if(visitService == null){
            visitService = createRetrofit().create(VisitService.class);
        }
        return visitService;
    }
